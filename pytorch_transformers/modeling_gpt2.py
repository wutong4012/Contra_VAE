...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def forward(self, input_ids, past=None, class_label=None, attention_mask=None, token_type_ids=None, position_ids=None, 
                head_mask=None, latent_as_gpt_emb=False, latent_as_gpt_memory=True, feats_embeds=None):
        ...
        if past is None:
            ...
            if latent_as_gpt_emb:
                ...
                if class_label is not None and self.nclasses > 0: # used for label-conditioned-VAE
                    past_emb += self.linear_class_emb(class_label)

            if latent_as_gpt_memory:
                past = self.linear(past)
                if class_label is not None and self.nclasses > 0: # used for label-conditioned-VAE
                    past += self.linear_class(class_label)
                share_latent = False
                if share_latent: 
        ...
        if feats_embeds is not None:
            hidden_states = hidden_states + feats_embeds # Attention!!
        if latent_as_gpt_emb:
            # pdb.set_trace()
...
class GPT2ForLatentConnector(GPT2PreTrainedModel):
    ...
    """
    def __init__(self, config, latent_size=32, nclasses=0, latent_as_gpt_emb=True, latent_as_gpt_memory=True):
        
        super(GPT2ForLatentConnector, self).__init__(config)

        
        self.transformer = GPT2Model(config, latent_size=latent_size, nclasses=nclasses)
        self.lm_head = nn.Linear(config.n_embd, config.vocab_size, bias=False)
    ...
    def forward(self, input_ids, past=None, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None,
                labels=None, class_label=None, label_ignore=None, loss_mask=None, feats_embeds=None):
